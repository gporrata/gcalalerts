...
- **Stop from the lock screen, no unlock needed:**
  - When the phone is locked or the screen is off, the screen turns on and a **full-screen alert** appears over
    the lock screen showing the meeting title, time and location, with a big green **STOP** button (plus **Snooze 5 min**).
...
The launcher icon is a Tibetan hand bell (ghanta) in brass on deep teal. It's an adaptive icon with a monochrome layer for
Android 13+ themed icons. The vectors are generated by `tools/gen_icon.py`; run `python3 tools/gen_icon.py /tmp/previews`
to regenerate them and render PNG previews (needs `rsvg-convert` and ImageMagick).
